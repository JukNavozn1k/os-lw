...
class HelpTab(ttk.Frame):
    ...
    def _fill_content(self):
        ...
        self._add_bullet("Введённые символы добавляются в очередь, из которой поток пользователя последовательно записывает их в файл.")
        self._add_h2("Управление")
        self._add_bullet("Можно менять задержку каждого потока отдельно.")
        self._add_bullet("В поле ввода можно быстро подставить примерный текст кнопкой 'Lorem ipsum'.")
        self._add_bullet("Файл можно выбрать вручную и очищать кнопкой 'Очистить'.")
        self._add_bullet("Просмотр содержимого можно обновить вручную кнопкой 'Обновить' или ждать автообновление.")
